...
def generate_event_flyer(
    ...
    **kwargs,  # noqa: ARG001
    ...
    """
    # Compat shim for upstream issue #4 — grade.py calls this function
    # with `pub_name=` while test_week1.py and the @tool stub use `venue_name=`.
    # Accept `pub_name` via **kwargs as an alias so both pass.
    # https://github.com/sovereignagents/sovereign-agent-lab/issues/4
    if not venue_name and "pub_name" in kwargs:
        venue_name = kwargs["pub_name"]
